...
async def get_user_guilds(token: str):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(DISCORD_API_ENDPOINT + "/users/@me/guilds", headers=headers) as resp:
...
async def get_guild_data(guild_id: int):
    async with aiohttp.ClientSession() as session:
        async with session.get(DISCORD_API_ENDPOINT + f"/guilds/{guild_id}", headers={"Authorization": f"Bot {BOT_TOKEN}"}) as resp:
            try:
                resp.raise_for_status()
